...
const initialState = {
    tasksList: [],
    selectedTask:{},
    isLoading:false,                    //api call success or false
    error:''                            //handle error API Calls
}

const BASE_URL = 'http://localhost:8000/tasks'

//GET 
//create thung action creatore
export const getTasksFromServer = createAsyncThunk(
    "tasks/getTasksFromServer",                                                     //Step-1   type (is a string)
    async (_,{rejectWithValue})=>{                                                  //Step-2   callback function
...
export const addTaskToServer = createAsyncThunk(
    "tasks/addTaskToServer",                                                     //Step-1   type (is a string)
...
export const updateTaskInServer = createAsyncThunk(
    "tasks/updateTaskInServer",                                                     //Step-1   type (is a string)
    async (task,{rejectWithValue})=>{ 
        const options = {
            method: "PATCH",
            body: JSON.stringify(task),
...
export const deleteTaskFromServer = createAsyncThunk(
    "tasks/deleteTaskFromServer",                                                     //Step-1   type (is a string)
    async (task,{rejectWithValue})=>{ 
        const options = {
            method: "DELETE",
        }                                                 //Step-2   callback function
...
const tasksSlice = createSlice({
    ...
    extraReducers:(builder) => {                    //standard formate
        builder
